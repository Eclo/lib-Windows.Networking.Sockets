...
else
{
    ...
    git add "source\Windows.Networking.Sockets\Properties\AssemblyInfo.cs"
...
if ($env:APPVEYOR_REPO_BRANCH -match "^dev*" -or $env:APPVEYOR_REPO_TAG -eq "true")
{
    ...
    # new branch name
    $newBranch = "$env:APPVEYOR_REPO_BRANCH-nfbot/update-version/Windows.Networking.Sockets/$env:GitVersion_NuGetVersionV2"
    ...
    $assemblyFiles = (Get-ChildItem -Path ".\*" -Include "win_net_sockets_native.cpp" -Recurse)
    ...
    {
        $commitMessage = "Update Windows.Networking.Sockets version to $env:GitVersion_NuGetVersionV2"
